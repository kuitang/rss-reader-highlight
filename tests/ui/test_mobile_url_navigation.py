...
class TestMobileNavigationComplete:
    ...
    def fresh_start(self, page: Page, test_server_url):
        ...
        page.goto(f"{test_server_url}/?unread=0", wait_until="networkidle", timeout=MAX_WAIT_MS)
    ...
    def test_scroll_position_preservation_critical(self, page: Page, test_server_url):
        ...
        page.goto(f"{test_server_url}/?unread=0", timeout=MAX_WAIT_MS)
        # Wait for specific mobile layout element
        page.wait_for_selector("#mobile-layout", state="visible", timeout=MAX_WAIT_MS)
        wait_for_page_ready(page)
        ...
        page.wait_for_url("**/item/**", timeout=MAX_WAIT_MS)
    ...
    def test_all_posts_vs_unread_state_preservation(self, page: Page, test_server_url):
        ...
        page.goto(f"{test_server_url}/", timeout=MAX_WAIT_MS)
        # Wait for specific mobile layout element
        page.wait_for_selector("#mobile-layout", state="visible", timeout=MAX_WAIT_MS)
        wait_for_page_ready(page)
